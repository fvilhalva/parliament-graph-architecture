...
%-------------------------------------------------------------------------------------%
\begin{resumo}
O presente trabalho propõe e implementa uma arquitetura modular e reprodutível para a análise de redes parlamentares da Câmara dos Deputados do Brasil, fundamentada em Teoria dos Grafos, com o objetivo de transformar dados legislativos brutos em evidências analíticas sobre estruturas de influência política. A metodologia segue o paradigma \textit{Design Science Research} (DSR), aplicada à construção de um artefato de software estruturado em camadas independentes (extração, processamento, núcleo analítico, persistência e visualização), validado por 210 testes automatizados (cobertura total de 83\%, núcleo analítico $\geq$ 93\%). A rede de coautoria foi modelada como projeção unimodal de um grafo bipartido $B = (D \cup P,\, E)$, onde $D$ representa deputados e $P$ proposições legislativas, com arestas ponderadas pela função $w(i,j) = \sum_{p} 1/(n_p - 1)$. Quatro métricas de centralidade (grau, intermediação, proximidade e autovetor) e dois algoritmos de detecção de comunidades (Louvain e Label Propagation) foram aplicados aos quatro anos legislativos do período 2022--2025, abrangendo a transição entre a 56ª e a 57ª Legislaturas. A hipótese central (H1) --- de que a estrutura de comunidades da rede é não-aleatória --- foi testada por meio de um teste de permutação contra grafos nulos que preservam a sequência de graus (200 permutações), e a robustez das partições foi avaliada pela concordância entre os dois algoritmos via \textit{Adjusted Rand Index} (ARI). Os resultados confirmam a hipótese em todos os anos analisados: a modularidade observada ($Q_{\text{obs}} \in [0{,}596;\ 0{,}640]$) é significativamente superior à média dos grafos nulos ponderados de mesma sequência de graus ($Q_{\text{nulo}} \approx 0{,}42$), com $p < 0{,}005$ --- nenhuma das 200 permutações anuais atinge $Q_{\text{obs}}$ ---, demonstrando que a arquitetura captura de forma consistente a estrutura modular não-aleatória da rede ao longo dos quatro anos analisados. A arquitetura proposta demonstrou-se reprodutível, auditável e independente da fonte de dados, constituindo ferramenta científica reutilizável para análises legislativas computacionais.
...
This work proposes and implements a modular and reproducible architecture for the analysis of parliamentary networks of the Brazilian Chamber of Deputies, grounded in Graph Theory, aiming to transform raw legislative data into analytical evidence about political influence structures. The methodology follows the \textit{Design Science Research} (DSR) paradigm, applied to the construction of a software artifact organized into independent layers (extraction, processing, analytical core, persistence and visualization), validated by 210 automated tests (83\% total coverage, analytical core $\geq$ 93\%). The co-authorship network was modeled as a one-mode projection of a bipartite graph $B = (D \cup P,\, E)$, where $D$ represents deputies and $P$ legislative propositions, with edges weighted by the function $w(i,j) = \sum_{p} 1/(n_p - 1)$. Four centrality metrics (degree, betweenness, closeness and eigenvector) and two community detection algorithms (Louvain and Label Propagation) were applied to the four legislative years of the 2022--2025 period, spanning the transition between the 56th and 57th Legislatures. The central hypothesis (H1) --- that the network's community structure is non-random --- was tested through a permutation test against null graphs preserving degree sequence (200 permutations), and partition robustness was assessed via the agreement between both algorithms using the \textit{Adjusted Rand Index} (ARI). Results confirm the hypothesis in every analysed year: the observed modularity ($Q_{\text{obs}} \in [0.596;\ 0.640]$) is significantly higher than the mean of degree-preserving weighted null graphs ($Q_{\text{null}} \approx 0.42$), with $p < 0.005$ --- no annual permutation out of 200 reaches $Q_{\text{obs}}$ ---, demonstrating that the architecture consistently captures the network's non-random modular structure across the four analysed years. The proposed architecture proved to be reproducible, auditable and source-agnostic, constituting a reusable scientific tool for computational legislative analysis.
...
A Teoria dos Grafos oferece precisamente esse instrumental. Ao modelar a coautoria legislativa como uma rede ponderada, na qual deputados são vértices e colaborações em proposições são arestas, torna-se possível aplicar métricas consolidadas de centralidade, algoritmos de detecção de comunidades e testes estatísticos de significância. Essa abordagem, inaugurada no contexto legislativo por \citeauthor{fowler2006} \cite{fowler2006} no estudo do Congresso dos Estados Unidos, transforma assinaturas em documentos oficiais em um objeto matemático tratável, do qual se podem extrair evidências mensuráveis sobre poder informal, coalizões e mediação política.

O presente trabalho parte da premissa de que a coautoria parlamentar não é fenômeno aleatório, mas reflexo de alinhamentos políticos, ideológicos e estratégicos. Sob essa premissa --- e na esteira de \citeauthor{fowler2006} \cite{fowler2006} ---, a rede de coautoria é adotada como \textit{proxy} mensurável de articulação política, passível de análise quantitativa rigorosa. A cobertura estende-se deliberadamente de 2022 a 2025 --- abrangendo a transição entre a 56ª e a 57ª Legislaturas ---, permitindo tanto a análise da estrutura em regime estável quanto o estudo da sua persistência ao longo do tempo.
...
A pergunta de pesquisa central que orienta este trabalho é formulada em termos estritamente estruturais (topológicos) --- a partir de uma propriedade mensurável do grafo, a modularidade --- e sem recurso a qualquer classificação ideológica atribuída aos partidos: \textit{a estrutura de comunidades observada na rede de coautoria parlamentar é significativamente diferente do que se esperaria por acaso?}

A essa pergunta associa-se uma hipótese falsificável, enunciada abaixo como predição sobre a estrutura da rede. O procedimento que a operacionaliza e a testa é descrito no Capítulo~\ref{cap:metodologia}, e o veredito correspondente --- corroboração ou refutação --- é apresentado no Capítulo~\ref{cap:resultados}.

\begin{itemize}
    \item \textbf{H1 - Única Hipótese (Estrutura não-aleatória)}: a rede de coautoria parlamentar apresenta estrutura de comunidades não-aleatória, com modularidade $Q$ substancialmente superior à esperada em redes com a mesma sequência de graus e ligações redistribuídas ao acaso.
\end{itemize}
...
Na sua forma mais elementar, uma rede (ou grafo, na literatura matemática) é um catálogo dos componentes de um sistema e das interações diretas entre eles. Conforme \cite{barabasi2016, diestel2017}, esses componentes são frequentemente chamados de nós (ou vértices) e as interações são chamadas de ligações (ou arestas), conforme a formulação matemática a seguir:
...
Com a topologia da rede computacionalmente definida, a Ciência de Redes atua quantificando a importância estrutural de cada vértice. Newman \cite{newman2010} aponta que grande parte da pesquisa em redes busca responder à pergunta: quais são os vértices mais importantes ou centrais em uma rede? Para responder a isso, existem diversas métricas matemáticas que capturam diferentes perspectivas do que significa ter influência. Neste trabalho, destacam-se quatro métricas principais:
...
A detecção de comunidades é o processo de encontrar divisões naturais em uma rede, buscando grupos de vértices onde existem muitas arestas internamente e poucas arestas conectando grupos distintos. Segundo \cite{newman2010}, diferentemente do particionamento de grafos tradicional (onde o número e o tamanho dos grupos são rigidamente predefinidos), a detecção de comunidades é uma ferramenta exploratória que busca descobrir a estrutura de larga escala de forma autônoma. No contexto deste projeto, as comunidades extraídas do grafo representam os agrupamentos parlamentares, revelando blocos partidários, facções informais ou fortes alinhamentos ideológicos no legislativo.
...
Valores de $Q$ variam tipicamente entre $-0{,}5$ e $1$. Conforme \cite{newman2010}, valores acima de $0{,}3$ indicam estrutura comunitária significativa em redes reais; valores próximos de zero indicam que a partição é estatisticamente indistinguível da aleatória; e valores negativos indicam anti-comunidades. A modularidade fornece, portanto, um critério objetivo para comparar partições alternativas geradas por diferentes algoritmos.
...
No artigo original do Louvain \cite{blondel2008}, os autores apresentam a fórmula exata usada na primeira fase do algoritmo. Ela calcula, com baixíssimo custo computacional, qual seria a variação de modularidade ($\Delta Q$), ou ganho de modularidade, ao mover um nó isolado $i$ para dentro de uma comunidade vizinha $C$:
...
No livro \textit{Networks} \cite{newman2010}, Mark Newman demonstra que, para fins de particionamento de grafos e otimização espectral, a modularidade pode ser reescrita elegantemente em notação matricial:
...
O paradigma metodológico adotado neste trabalho é o \textit{Design Science Research} (DSR), conforme formalizado por Hevner et al. \cite{hevner2004} e Wieringa \cite{wieringa2014}. Diferentemente das ciências naturais, que explicam fenômenos existentes, e das ciências sociais empíricas, que descrevem comportamentos observáveis, o DSR é orientado à \textbf{produção de artefatos} (constructos, modelos, métodos ou instanciações de software) projetados para solucionar problemas práticos identificados em um contexto real \cite{hevner2004, wieringa2014}.

Como ilustrado no framework clássico de Hevner et al. \cite{hevner2004} (Figura~\ref{fig:dsr_framework}), a condução desta pesquisa estabelece uma sinergia rigorosa entre três domínios fundamentais:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{NetworkX 3.2} \cite{networkx}: representação e algoritmos de grafos;
    \item \textbf{pandas 2.2}: manipulação tabular dos CSVs;
    \item \textbf{python-louvain} \cite{blondel2008}: detecção de comunidades pelo algoritmo Louvain;
    \item \textbf{matplotlib} e \textbf{seaborn}: visualização e geração de figuras;
    \item \textbf{pydantic} e \textbf{pydantic-settings}: validação de tipos e carregamento tipado da configuração a partir do arquivo \texttt{.env};
...
Para mitigar esse viés de escala e capturar a verdadeira intensidade da colaboração política, adota-se a metodologia de projeção ponderada proposta por Newman \cite{newman2010}:
...
obtida pelo Label Propagation por meio do \textit{Adjusted Rand Index} (ARI). O
...
\begin{center}
\texttt{extraction $\rightarrow$ processing $\rightarrow$ core (Graph + Algorithms) $\rightarrow$ repository $\rightarrow$ visualization}
\end{center}
...
A camada \texttt{extraction/} contém a classe \texttt{ChamberExtractor}, responsável pelo download e cache dos CSVs do Portal de Dados Abertos. Implementa: (i) verificação de existência local antes do download, reutilizando o arquivo em cache quando presente; (ii) download direto dos arquivos CSV do portal via \texttt{pandas}; (iii) persistência do arquivo no diretório de cache local; (iv) reaproveitamento determinístico do cache em execuções subsequentes, tornando a série reprodutível sem depender da rede.
...
A Figura~\ref{fig:juncao_dados} sintetiza o encadeamento das operações. O cruzamento
