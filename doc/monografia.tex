...
\begin{epigrafe}
    \vspace*{\fill}
    \begin{flushright}
        \textit{``Devemos o baixo custo da eletricidade hoje à rede elétrica, o sistema que surgiu
         por meio dessas conexões entre pares, unindo todos os produtores e consumidores
         em uma única rede. Ela permite que a energia produzida a baixo custo seja instantaneamente
         transportada para qualquer lugar. Assim, a eletricidade oferece um exemplo maravilhoso do
         enorme impacto positivo que as redes exercem sobre as nossas vidas.''}\\
        \vspace*{0.3cm}
        --- Albert-László Barabási, \textit{Network Science}
    \end{flushright}
\end{epigrafe}
...
O presente trabalho parte da premissa de que a coautoria parlamentar não é fenômeno aleatório, mas reflexo de alinhamentos políticos, ideológicos e estratégicos. Sob essa premissa, a rede de coautoria constitui um \textit{proxy} válido e mensurável de articulação política, passível de análise quantitativa rigorosa. A cobertura estende-se deliberadamente de 2022 a 2025 --- abrangendo a transição entre a 56ª e a 57ª Legislaturas ---, permitindo tanto a análise da estrutura em regime estável quanto o estudo da sua persistência ao longo do tempo.
...
\begin{itemize}
    \item \textbf{H1 - Unica Hipótese (Estrutura não-aleatória)}: a rede de coautoria parlamentar apresenta estrutura de comunidades não-aleatória, com modularidade $Q$ substancialmente superior à esperada em redes com a mesma sequência de graus e ligações redistribuídas ao acaso.
\end{itemize}
...
Como ilustrado no framework clássico de Hevner et al. \cite{hevner2004} (~\ref{fig:dsr_framework}), a condução desta pesquisa estabelece uma sinergia rigorosa entre três domínios fundamentais:
\begin{itemize}
...
\begin{figure}[H]
    ...
    \fonte{Gerado por Felipe no Graphviz Online (2026).}
\end{figure}
...
A~\ref{fig:juncao_dados} sintetiza o encadeamento das operações. O cruzamento
...
As Propostas de Lei Complementar, por exigirem maioria absoluta para aprovação e tratarem de matérias constitucionalmente reservadas, tendem a apresentar coautorias mais selecionadas. No dataset analisado, sua presença numérica é menor que a de PLs, mas sua contribuição estrutural à rede é comparativamente densa.
