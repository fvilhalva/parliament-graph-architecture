...
\begin{itemize}
    ...
    \item \textbf{requests}: cliente HTTP para download dos CSVs da Câmara;
    \item \textbf{python-dotenv}: carregamento de variáveis de ambiente do arquivo \texttt{.env};
...
A camada \texttt{extraction/} contém a classe \texttt{ChamberExtractor}, responsável pelo download e cache dos CSVs do Portal de Dados Abertos. Implementa: (i) verificação de existência local antes do download; (ii) requisição HTTP com timeout configurável; (iii) escrita atômica no diretório de cache; (iv) tratamento de erros de rede com mensagens descritivas.
