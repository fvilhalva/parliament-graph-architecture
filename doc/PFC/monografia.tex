    ...
    % microtype carregado pelo pacoteBasico; o memoir/abntex2 já redefine footnote,
    % então desativamos os "patches" (evita o warning 'Unable to apply patch footnote').
    % Protrusion/expansion (o essencial do microtype) continuam ativos.
    ...
    % O template (pacoteBasico.sty) imprime \subtituloDoTrabalho na capa e na
    % folha de rosto; como este trabalho não tem subtítulo, definimos vazio
    % (deixá-lo indefinido causa "Undefined control sequence").
    \newcommand{\subtituloDoTrabalho}{}
    \newcommand{\tipoDeTrabalho}{Projeto Final de Curso}
    ...
    % Banca = 3 membros (orientador + 2), conforme Normas Complementares.
    % Preencher nomes/titulações reais quando a banca for definida; os DOIS
    % membros abaixo devem ficar em ORDEM ALFABÉTICA (o orientador vem primeiro).
    % Titulações: Dr./Dra. (doutor), MSc. (mestre), Esp. (especialista).
    ...
    % Membro C: opcional (suplente). Não entra na banca de 3; manter apenas se necessário.
    ...
    %-------------------------------------------------------------------------------------%
    % P Á G I N A   ii  --  F I C H A   C A T A L O G R Á F I C A
    %-------------------------------------------------------------------------------------%
    % A ficha catalográfica é fornecida pela Biblioteca da UEMS
    % (fichacatalografica@uems.br) SOMENTE após a versão final corrigida
    % (pós-defesa). Até lá, não há o que inserir aqui.
    % Na VERSÃO FINAL, descomentar o bloco abaixo e inserir a ficha recebida:
    %
    % \newpage
    % \thispagestyle{empty}
    % \vspace*{\fill}
    % \begin{center}
    %     \includegraphics[width=12.5cm]{ficha-catalografica.png}
    % \end{center}

    %-------------------------------------------------------------------------------------%
    % P Á G I N A   iii  --  F O L H A   D E   A P R O V A Ç Ã O  (B A N C A)
    %-------------------------------------------------------------------------------------%
    % Formato do modelo UEMS ("9d - iii"): listagem sem linhas de assinatura;
    % orientador (presidente) primeiro, demais membros em ordem alfabética;
    % cada um com "Instituição:" e "Área:".
        \begin{folhadeaprovacao}
            ...
            \begin{center}
                ...
                {\bfseries\tituloDoTrabalho}
            \end{center}

            \vspace*{\fill}

            \noindent\textbf{Banca Examinadora:}\par
            \vspace*{0.8cm}

            \noindent\tituloDoOrientador\ \nomeDoOrientador\ (Orientador)\\
            Instituição: \instituicaoorientador\\
            Área: Computação\par
            \vspace*{0.8cm}

            \noindent\tituloDoMembroA\ \nomeDoMembroA\\
            Instituição: \instituicaomembroA\\
            Área: Computação\par
            \vspace*{0.8cm}

            \noindent\tituloDoMembroB\ \nomeDoMembroB\\
            Instituição: \instituicaomembroB\\
            Área: Computação\par
    ...
    Este trabalho aborda a lacuna entre a abundância de dados legislativos abertos da Câmara dos Deputados do Brasil e a escassez de ferramentas analíticas reprodutíveis capazes de extrair, desses dados, evidências mensuráveis sobre estruturas de influência política. Sob o paradigma de \textit{Design Science Research}, projetou-se, implementou-se e validou-se uma arquitetura de software modular e auditável que transforma registros de coautoria legislativa do período 2022--2025 em uma rede analisável por Teoria dos Grafos. A rede é modelada como projeção unimodal ponderada de um grafo bipartido, adotando o peso $1/(n_p-1)$ e um filtro de massa de signatários (\texttt{max\_authors}$=30$) para mitigar a inflação de arestas. Sobre a rede resultante, calculam-se quatro métricas de centralidade e detectam-se comunidades por dois algoritmos independentes --- Louvain e Label Propagation ---, cuja significância é avaliada por teste de permutação contra modelo nulo ponderado. A hipótese central, de que a estrutura de comunidades é não-aleatória, foi corroborada nos quatro anos analisados ($p<0{,}005$), com modularidade observada entre $0{,}596$ e $0{,}640$; as comunidades detectadas recuperam a organização partidária no nível de coalizão (\textit{Adjusted Rand Index} $\approx 0{,}30$); a articulação legislativa concentra-se em poucos parlamentares; e a centralidade de coautoria revela-se medida de articulação, não de poder institucional formal. A contribuição central, coerente com o paradigma adotado, é o próprio artefato: uma ferramenta reprodutível, coberta por testes automatizados e empacotada em contêiner, disponibilizada em código aberto para auditoria, extensão e reuso.

        \vspace*{0.5cm}
        \noindent\textbf{{Palavras-Chave: }} ciência de redes; ciência social computacional; engenharia de software; teoria dos grafos; detecção de comunidades; redes parlamentares.
    \end{resumo}
    ...
    This work addresses the gap between the abundance of open legislative data from the Brazilian Chamber of Deputies and the scarcity of reproducible analytical tools able to extract, from such data, measurable evidence about structures of political influence. Under the \textit{Design Science Research} paradigm, a modular and auditable software architecture was designed, implemented and validated to transform legislative co-authorship records from the 2022--2025 period into a network analyzable through Graph Theory. The network is modeled as a weighted one-mode projection of a bipartite graph, adopting the weight $1/(n_p-1)$ and a signatory-mass filter (\texttt{max\_authors}$=30$) to mitigate edge inflation. Over the resulting network, four centrality metrics are computed and communities are detected by two independent algorithms --- Louvain and Label Propagation ---, whose significance is assessed by a permutation test against a weighted null model. The central hypothesis, that the community structure is non-random, was corroborated in all four analyzed years ($p<0.005$), with observed modularity between $0.596$ and $0.640$; the detected communities recover the partisan organization at the coalition level (\textit{Adjusted Rand Index} $\approx 0.30$); legislative articulation is concentrated in a few members; and co-authorship centrality proves to be a measure of articulation, not of formal institutional power. The central contribution, consistent with the adopted paradigm, is the artifact itself: a reproducible tool, covered by automated tests and packaged in a container, released as open source for auditing, extension and reuse.

        \vspace*{0.5cm}
        \noindent\textbf{{Keywords: }} network science; computational social science; software engineering; graph theory; community detection; parliamentary networks.
    \end{otherlanguage*}
    \end{resumo}
    ...
    Este trabalho aborda essas lacunas por meio da construção de uma \emph{arquitetura modular, reprodutível e auditável} para análise de redes parlamentares, fundamentada em Teoria dos Grafos e Engenharia de Software contemporânea, aplicada aos quatro anos legislativos completos do período 2022--2025.
    
    \section{Objetivos}
    Esta seção apresenta o objetivo geral do projeto e os objetivos específicos que dele decorrem.
    ...
    \begin{enumerate}[label=\alph*)]
        \item Modelar matematicamente a rede de coautoria legislativa como projeção unimodal de um grafo bipartido, com fórmula de ponderação justificada teoricamente;
         \item Implementar \textit{pipeline} computacional em camadas independentes, seguindo princípios de Arquitetura Limpa e SOLID, com cobertura por testes automatizados;
    ...
    Essa caracterização é organizada em torno de três \emph{perguntas norteadoras}, de natureza descritiva e exploratória --- e não hipóteses a serem testadas estatisticamente:
    ...
    \noindent Diferentemente de H1, essas questões não são submetidas a teste de significância: são respondidas pela caracterização empírica da rede no Capítulo~\ref{cap:resultados}. A hipótese H1 é, ainda assim, a \emph{condição que as viabiliza} --- só faz sentido perguntar se as comunidades correspondem a partidos, por exemplo, depois de demonstrado que a estrutura comunitária não é fruto do acaso.
    
    \section{ESTRUTURA DO TRABALHO}
    
    O restante deste trabalho está organizado da seguinte forma. O Capítulo~\ref{cap:revisao} apresenta a revisão da literatura, cobrindo os fundamentos de Engenharia de Software, Teoria dos Grafos, Ciência de Redes e Detecção de Comunidades. O Capítulo~\ref{cap:metodologia} descreve a metodologia, incluindo o paradigma DSR, os procedimentos de coleta e tratamento de dados, a estratégia de modelagem, a medida de robustez das partições e os procedimentos de validação. O Capítulo~\ref{cap:desenvolvimento} detalha o desenvolvimento do artefato de software, com ênfase na arquitetura modular e nas decisões de implementação. O Capítulo~\ref{cap:resultados} apresenta os resultados obtidos para os quatro anos analisados, com a caracterização das centralidades e o teste da hipótese central. O Capítulo~\ref{cap:conclusao} conclui o trabalho com síntese dos resultados, discussão das limitações e proposição de trabalhos futuros.
...
\label{cap:revisao}
    ...
    \begin{figure}[H]
        ...
        \label{fig:ac_limpa}
        \includegraphics[width=0.6\textwidth]{imagens/revisao/arquitetura-limpa.png}
    ...
    \begin{enumerate}[label=\alph*)]
        ...
        \item \textbf{Extraction, Processing e Visualization}: atuam como Adaptadores de Interface, consumindo dados brutos do \textit{dataset} da Câmara e processando visualizações gráficas de maneira desacoplada das regras de negócio;
    ...
    \begin{figure}[H]
        ...
        \label{fig:grafo_1}
        \includegraphics[width=0.6\textwidth]{imagens/revisao/grafo-exemplo.png}
        \fonte{Diestel (2017, p. 2).}
    \end{figure}
    
    A topologia de uma rede é ditada pela natureza de suas arestas. \citeonline{barabasi2016} explica que os grafos podem ser direcionados, quando a relação possui um sentido único (como um link na World Wide Web), ou não direcionados, quando a relação é mútua e de via dupla (como uma linha de transmissão de energia). Além da direção, as redes do mundo real frequentemente possuem conexões com intensidades diferentes. Para modelar essas intensidades, utilizam-se os grafos ponderados, onde cada aresta recebe um peso numérico.
    ...
    \begin{figure}[H]
        ...
        \includegraphics[width=0.6\textwidth]{imagens/revisao/grafo-bipartido.png}
        \fonte{Diestel (2017, p. 15).}
    \end{figure}
    
    Como grafos bipartidos representam dois tipos distintos de entidade simultaneamente, sua análise direta por métricas de centralidade convencionais torna-se pouco intuitiva. A técnica usual consiste em derivar, a partir do grafo bipartido, uma rede unimodal (\textit{one-mode}) por meio de uma operação de \emph{projeção}. Na projeção sobre o subconjunto $U$, dois vértices de $U$ passam a ser conectados sempre que compartilham ao menos um vizinho comum em $V$ \cite{newman2010}. Transpondo essa definição para o objeto deste trabalho, a projeção sobre o conjunto de deputados conecta dois parlamentares sempre que ambos assinam uma mesma proposição legislativa, produzindo a rede de coautoria efetivamente analisada.
    ...
    \begin{figure}[H]
        ...
        \label{fig:hollywood}
        \includegraphics[width=0.6\textwidth]{imagens/revisao/rede-hollywood.png}
        \fonte{Barabási (2016).}
    \end{figure}
    ...
    \begin{enumerate}[label=\alph*)]
        ...
        \item \textbf{Centralidade de Intermediação (\textit{Betweenness})}: desenvolvida inicialmente pelo sociólogo Linton Freeman (\citeyear{freeman1977}), esta métrica avalia a frequência com que um vértice atua como uma ponte ao longo dos caminhos mínimos (geodésicos) entre outros pares de vértices na rede. Matematicamente, ela é a razão entre a quantidade de caminhos mínimos que passam pelo vértice em questão e o número total de caminhos mínimos que conectam cada par de nós:
        ...
        onde $n$ é o número de vértices e $d(v,u)$ é a distância geodésica entre $v$ e $u$; ou seja, $C_C(v)$ é o inverso da distância geodésica \emph{média} do vértice $v$ aos demais vértices da rede.
    ...
    A modularidade é a métrica formal mais consolidada para avaliar a qualidade de uma partição em comunidades em uma rede. Proposta originalmente por \citeonline{newman2004} e formalizada matematicamente em \citeonline{newman2010}, a modularidade $Q$ mede a diferença entre a fração de arestas que conectam vértices da mesma comunidade no grafo observado e a fração esperada em um grafo aleatório com a mesma sequência de graus. Sua formulação matemática é:
    ...
    Um aspecto central da formulação da modularidade merece destaque, pois fundamenta o procedimento de validação estatística adotado neste trabalho. O termo $k_i k_j / 2m$ presente na Equação~\ref{eq:modularidade} não é arbitrário: ele representa o peso ou intensidade esperada da conexão entre os vértices $i$ e $j$ em um \emph{modelo nulo}, isto é, em um grafo aleatório que preserva a sequência de graus da rede original mas redistribui as conexões ao acaso \cite{newman2010}. A modularidade mede, portanto, exatamente o excesso de arestas internas às comunidades em relação ao que se esperaria por puro acaso, dada a mesma distribuição de graus.
    ...
    É justamente nesse cruzamento que reside a lacuna endereçada: são raros, no contexto brasileiro, os estudos que combinam análise formal de redes de coautoria, cobertura multi-anual recente e disponibilização de código-fonte auditável. O presente trabalho posiciona-se nessa interseção, aproximando o instrumental consolidado por \citeonline{fowler2006} da realidade da Câmara dos Deputados do Brasil, com ênfase em reprodutibilidade computacional.
    ...
    O paradigma metodológico adotado neste trabalho é o \textit{Design Science Research} (DSR), conforme formalizado por \citeonline{hevner2004} e \citeonline{wieringa2014}. Diferentemente das ciências naturais, que explicam fenômenos existentes, e das ciências sociais empíricas, que descrevem comportamentos observáveis, o DSR é orientado à \emph{produção de artefatos} (constructos, modelos, métodos ou instanciações de software) projetados para solucionar problemas práticos identificados em um contexto real.
    
    Como ilustrado no framework clássico de \citeonline{hevner2004} (\ref{fig:dsr_framework}), a condução desta pesquisa estabelece uma sinergia entre três domínios fundamentais:
    \begin{itemize}
        ...
        \item O \textbf{Espaço de Pesquisa} em si, focado na construção (implementação do \textit{pipeline} analítico em Python) e na avaliação iterativa do artefato gerado.
    \end{itemize}
    ...
    \begin{figure}[H]
        ...
        \includegraphics[width=0.75\textwidth]{imagens/metodologia/dsr-framework.png}
        \fonte{Adaptado de \citeonline[p. 80]{hevner2004}.}
    \end{figure}
    
    Para orquestrar essas frentes, a investigação estrutura-se em torno de três ciclos dinâmicos propostos na literatura de DSR \citeonline{hevner2004}: o \textit{ciclo de relevância}, que conecta o ambiente do problema às exigências do artefato; o \textit{ciclo de rigor}, que fundamenta o desenvolvimento na base de conhecimento existente; e o \textit{ciclo de design} propriamente dito, que executa iterativamente as fases de construção e avaliação até que o artefato atinja a maturidade necessária para responder ao problema analítico mapeado.
    ...
    \begin{enumerate}[label=\alph*)]
        ...
        \item \textbf{Definição de objetivos}: estabelecimento dos requisitos do artefato --- modularidade, reprodutibilidade, cobertura por testes, interoperabilidade com ferramentas externas (Gephi), independência da fonte de dados e independência do formato de persistência, esta última assegurada por interfaces (contratos) que desacoplam o núcleo analítico dos detalhes de exportação;
    
        \item \textbf{Projeto e desenvolvimento}: construção iterativa do software seguindo Arquitetura Limpa \citeonline{martin2017} e princípios SOLID, com cobertura por testes automatizados desde o início;
    
        \item \textbf{Demonstração}: execução do \textit{pipeline} completo para os anos 2022, 2023, 2024 e 2025, gerando os artefatos de dados (CSV, GEXF, SQLite e JSON);
    ...
    Em conformidade com as orientações de integridade acadêmica, registra-se que ferramentas de inteligência artificial generativa, baseadas em modelos de linguagem, foram utilizadas de forma auxiliar durante a elaboração deste trabalho. Seu uso englobou: apoio à revisão gramatical e estruturação textual; auxílio à compreensão e discussão de conceitos técnicos; e suporte na escrita, otimização e refatoração de trechos de código-fonte. A concepção da arquitetura de software, as decisões metodológicas e de modelagem, a validação lógica do sistema, a interpretação dos resultados e a autoria intelectual do trabalho são de responsabilidade exclusiva do autor.
    
    \section{PROCEDIMENTOS DE COLETA E TRATAMENTO DE DADOS}
    \label{sec:procedimentos}
    
    Para subsidiar a modelagem da rede parlamentar e viabilizar o cálculo de centralidades e a identificação de comunidades, estruturou-se um \textit{pipeline} sistemático de aquisição e processamento de dados. Esse fluxo operacional compreende etapas sequenciais que vão desde o consumo automatizado das bases públicas oficiais até a persistência das redes resultantes em formatos padronizados de alta interoperabilidade. Em conformidade com as diretrizes de desacoplamento da Arquitetura Limpa, todo o fluxo de ingestão e higienização de dados é tratado de maneira isolada do núcleo analítico da aplicação. Essa separação física e lógica entre as rotinas de extração (\textit{I/O}) e as regras de negócio assegura a integridade das métricas calculadas e garante a reprodutibilidade estrita do \textit{pipeline} para toda a série temporal analisada, compreendida entre os anos de 2022 e 2025.
    
    \subsection{Fonte dos Dados}
    
    A fonte primária de dados é o Portal de Dados Abertos da Câmara dos Deputados (\url{https://dadosabertos.camara.leg.br}), mantido pela própria instituição em cumprimento à Lei de Acesso à Informação (Lei 12.527/2011). Trata-se da \emph{fonte oficial primária} dos registros legislativos brasileiros, sem autoridade superior de referência. Eventuais inconsistências da fonte representam limitação intrínseca ao registro público nacional e estão fora do escopo deste trabalho.
    ...
    A delimitação do período em 2022 a 2025 resulta de critérios técnicos, e não de conveniência. Primeiro, \emph{estabilidade e homogeneidade do esquema de dados}: os arquivos CSV anuais do Portal de Dados Abertos mantêm formato e cobertura consistentes no período recente, evitando a heterogeneidade de esquema e a degradação de completude observadas em séries mais antigas. Segundo, \emph{coerência institucional}: o intervalo abrange uma legislatura quase completa (a 57\textsuperscript{a}, iniciada em 2023) somada ao ano de transição (2022, encerramento da 56\textsuperscript{a}), permitindo testar a \textit{persistência} da estrutura sob um regime legislativo estável, sem o confundimento de múltiplas mudanças de regras eleitorais e de composição partidária que um intervalo mais amplo (por exemplo, 2010 a 2025) introduziria. Terceiro, \emph{viabilidade computacional e de auditoria}: quatro \textit{snapshots} anuais completos são suficientes para avaliar a estabilidade do padrão estrutural (H1) mantendo cada grafo integralmente reproduzível e inspecionável. A ampliação da série temporal é registrada como trabalho futuro.
    ...
    \begin{itemize}
        ...
        \item \textbf{NetworkX 3.2} \cite
        {networkx}: representação, algoritmos de grafos e detecção de comunidades (Louvain e Label Propagation nativos);
    ...
    O ecossistema legislativo da Câmara dos Deputados forma naturalmente um \emph{grafo bipartido} $B = (V_B, E_B)$ \cite{barabasi2016, newman2010}, onde o conjunto de vértices é particionado em dois subconjuntos disjuntos, $V_B = D \cup P$ com $D \cap P = \emptyset$ \cite{diestel2017}:
    ...
    \begin{figure}[H]
        ...
        \includegraphics[width=0.55\textwidth]{imagens/metodologia/bipartido-coautoria.png}
        \fonte{Elaborado pelo autor usando Graphviz Online (2026).}
    \end{figure}
    
    Esse grafo bipartido é transformado, por meio de uma \emph{projeção unimodal} (ou de modo único) sobre o conjunto de deputados $D$, em uma rede de coautoria unimodal $G = (D, E', w)$ \cite{newman2010, barabasi2016}. Nessa projeção, dois deputados $i, j \in D$ são conectados por uma aresta em $E'$ se e somente se ambos assinarem pelo menos uma proposição em comum ($p \in P$), e o peso da aresta $w(i,j)$ demonstra a intensidade dessa colaboração legislativa com base na relevância das matérias compartilhadas.

    Explicita-se, para evitar ambiguidade, a natureza do objeto matemático analisado: a rede de coautoria $G$ é um \emph{grafo simples} (sem laços ou arestas múltiplas --- coautorias repetidas entre o mesmo par acumulam-se no peso, não em arestas paralelas), \emph{não-direcionado} (a coautoria é uma relação simétrica) e \emph{ponderado} (cada aresta carrega o peso $w(i,j)$ da Equação~\ref{eq:peso_coautoria}). Cada ano é modelado como um \textit{snapshot} \emph{estático} independente; a dimensão temporal do estudo (Seção~\ref{sec:temporal}) não decorre de um grafo dinâmico único, mas da \emph{comparação} entre os quatro grafos anuais construídos de forma idêntica.
    
    \begin{figure}[H]
        \centering
        \caption{Projeção unimodal do grafo bipartido da~\ref{fig:bipartid_coautoria}
         sobre o conjunto $D$, com os pesos calculados pela
         Equação~\ref{eq:peso_coautoria}.}
        \label{fig:bipartid_projecao_unimodal}
        \includegraphics[width=0.5\textwidth]{imagens/metodologia/projecao-unimodal.png}
        \fonte{Elaborado pelo autor usando Graphviz Online (2026).}
    \end{figure}
    
    A~\ref{fig:bipartid_projecao_unimodal} ilustra essa operação sobre o
    ...
    Para mitigar esse viés de escala e capturar a verdadeira intensidade da colaboração política, adota-se a metodologia de projeção ponderada proposta por \citeonline{newman2001,newman2010}:
    ...
    \begin{figure}[H]
        ...
        \includegraphics[width=0.5\textwidth]{imagens/metodologia/projecao-ponderada.png}
        \fonte{Elaborado pelo autor usando Graphviz Online (2026).}
    \end{figure}
    ...
    \begin{enumerate}[label=\alph*)]
        ...
        \item \textbf{Pesos uniformes por tipo de proposição}: Embora que durante o desenvolvimento desse trabalho tenha-se experimentado pesos diferenciados por tipo (por exemplo, PEC com peso maior que PL), não existe na literatura uma escala teoricamente justificada para tal ponderação. Adota-se, portanto, peso igual a 1 para todos os tipos, sendo a seleção qualitativa realizada pelo filtro de tipos descrito acima.
    \end{enumerate}
    ...
    Antes de aplicar essas métricas, é necessário entender o que se entende por influência neste trabalho. \emph{Influência política é aqui operacionalizada, em sentido estritamente estrutural, como a posição de um deputado na rede de coautoria} --- e não como poder institucional formal (relatorias, presidência de comissões, controle de pauta), que se exerce por mecanismos não capturados pela coautoria. Especificamente, a influência estrutural é mensurada por duas dimensões complementares: a intermediação (\textit{betweenness}), que capta o papel de ponte entre grupos, na tradição de \citeonline{freeman1977}; e a centralidade de autovetor, que capta a inserção em vizinhanças bem conectadas \cite{bonacich1972}. Trata-se, portanto, de uma medida de articulação e colaboração legislativa, não de poder decisório --- distinção retomada na discussão dos resultados.
    ...
    Cada métrica emprega, ademais, a representação coerente com sua semântica. Grau e autovetor são calculados sobre o grafo \emph{ponderado}, pois a intensidade da colaboração (afinidade) acumula-se de forma aditiva. Já intermediação e proximidade são calculadas sobre a topologia \emph{não-ponderada}: em grafos ponderados, o peso é interpretado como distância ou custo, ao passo que na rede de coautoria o peso representa afinidade --- semântica inversa, cuja conciliação exigiria uma transformação arbitrária $1/w$. A intermediação é \emph{normalizada} pelo fator $(n-1)(n-2)/2$ (grafos não-direcionados), produzindo valores em $[0,1]$ comparáveis entre anos de tamanhos distintos, requisito da análise temporal (Seção~\ref{sec:temporal}).
    ...
    O \textit{pipeline} é organizado em camadas com responsabilidades únicas, conforme o esquema:
    
    \begin{figure}[H]
        \centering
        \caption{Diagrama de componentes e fluxo de dados do \textit{pipeline}.}
        \label{fig:componentes}
        \includegraphics[width=0.6\textwidth]{imagens/desenvolvimento/diagrama-componentes.png}
    ...
    \begin{figure}[H]
        ...
        \includegraphics[width=0.6\textwidth]{imagens/desenvolvimento/fluxo-dados.png}
        \fonte{Elaborado pelo autor usando Graphviz Online (2026).}
    \end{figure}
    ...
    Para concretizar o Princípio da Inversão de Dependência (Seção~\ref{subsec:solid}), esses adaptadores não são referenciados diretamente pelas camadas superiores: cada um implementa um \emph{contrato} (interface) declarado no módulo \texttt{repository/interfaces.py} por meio de \textit{protocolos} (\texttt{typing.Protocol}). A Listagem~\ref{lst:interfaces} apresenta os contratos definidos.

\begin{lstlisting}[language=Python, caption={Contratos da camada de persistência (\texttt{repository/interfaces.py}).}, label={lst:interfaces}]
class MetricsRepository(Protocol):
    def export_deputy_metrics(self, deputies, year) -> Path: ...
    def export_coauthorship_metrics(self, coauthorships, year) -> Path: ...

class DatabaseRepository(Protocol):
    def export_deputy_metrics(self, deputies, year) -> Path: ...

class GraphRepository(Protocol):
    def export_gexf(self, graph, year=None) -> Path: ...
...
\end{lstlisting}
    ...
    A orquestração do \textit{pipeline} depende dessas abstrações, e não das classes concretas: a estrutura \texttt{PipelineDependencies} declara seus colaboradores de persistência pelos tipos dos protocolos, e a instanciação das implementações concretas ocorre em um único ponto de composição (\texttt{main.py}). Como consequência, trocar o formato de exportação (por exemplo, substituir o CSV por Parquet, ou o SQLite por outro banco) exige apenas fornecer uma nova classe que satisfaça o mesmo contrato, sem qualquer alteração no núcleo analítico. Trata-se de \emph{injeção de dependências} no ponto de composição, técnica que operacionaliza a inversão de controle e mantém o núcleo (\texttt{core/}) independente de detalhes de I/O --- coerente com a Regra da Dependência, verificável no código: o pacote \texttt{core/} não importa nenhuma das camadas externas.
    ...
    \subsection{Tipos de Proposição Considerados}

    Os cinco tipos admitidos pelo filtro qualitativo (Seção~\ref{subsec:heuristica}) têm perfis distintos de coautoria, o que justifica tratá-los individualmente:

    \begin{enumerate}[label=\alph*)]
        \item \textbf{Projetos de Lei (PL)}: a categoria mais numerosa e diversa do \textit{dataset}, com ampla variabilidade no número de coautores --- de proposições individuais a iniciativas com dezenas de signatários. Sob a heurística adotada, PLs com poucos coautores contribuem mais fortemente para as arestas, refletindo coordenação política mais específica;
        \item \textbf{Projetos de Lei Complementar (PLP)}: por exigirem maioria absoluta para aprovação e tratarem de matérias constitucionalmente reservadas, tendem a apresentar coautorias mais selecionadas; numericamente menos frequentes que os PLs, mas com contribuição estrutural comparativamente densa;
        \item \textbf{Propostas de Emenda à Constituição (PEC)}: os instrumentos de maior peso simbólico e os principais responsáveis pelo viés de massa de signatários, por exigirem 171 assinaturas (um terço da Câmara) para apresentação; a heurística de ponderação e o filtro de massa (Seção~\ref{subsec:heuristica}) mitigam e eliminam, respectivamente, esse efeito distorcivo sobre a topologia da rede;
        \item \textbf{Projetos de Decreto Legislativo (PDL)}: tratam de matérias de competência exclusiva do Congresso Nacional que dispensam sanção presidencial --- por exemplo, a aprovação de tratados internacionais e a sustação de atos normativos do Poder Executivo. Apresentam volume reduzido frente aos PLs, com padrão de coautoria mais institucional e menos numeroso;
        \item \textbf{Emendas (EMC)}: modificações propostas a uma proposição já em tramitação, tipicamente na fase de comissão. Por incidirem sobre um texto específico e não constituírem matéria autônoma, concentram-se predominantemente em poucos autores.
    \end{enumerate}
    
    \section{IMPLEMENTAÇÃO DO NÚCLEO ANALÍTICO COMPUTACIONAL}

    Definida a modelagem do grafo, esta seção detalha a instanciação do núcleo analítico na camada \texttt{core/}, onde reside a lógica de mais alta criticidade do artefato. Coerentemente com a Regra da Dependência, esse núcleo é isolado de qualquer preocupação de entrada e saída: recebe o grafo já construído e devolve resultados tipados, sem conhecer a origem dos dados nem o destino das saídas. As subseções a seguir descrevem, nessa ordem, o cálculo das quatro métricas de centralidade, a detecção de comunidades pelos dois algoritmos adotados e a persistência dos resultados analíticos em um objeto único e auditável.
    ...
    \begin{itemize}
        ...
        \item Intermediação: algoritmo de \citeonline{brandes2001}, complexidade $O(|V| \cdot |E|)$ em grafos esparsos;
        \item Proximidade: implementação sobre a topologia não-ponderada, coerente com a justificativa da Seção~\ref{subsec:metricas};
    ...
    Capítulo~\ref{cap:resultados}, eliminando a necessidade de reexecutar o \textit{pipeline}
    para obter qualquer número reportado.
    
    \section{QUALIDADE, INFRAESTRUTURA E REPRODUTIBILIDADE}

    A confiabilidade do artefato --- requisito central no paradigma DSR --- é sustentada por três pilares descritos nesta seção: uma suíte de testes automatizados que valida tanto o código quanto os dados por ele gerados; o empacotamento do ambiente de execução em contêiner Docker, que assegura replicação exata; e a disponibilização do código-fonte em repositório aberto, condição para a auditabilidade e o reuso do trabalho por terceiros.

    \subsection{Suíte de testes}
    \label{subsec:suite}
    
    O sistema é coberto por 217 testes automatizados, distribuídos em módulos que
    ...
    integridade dos \textit{datasets} gerados. Um subconjunto de 64 testes de integridade é
    parametrizado por ano e verifica, sobre os arquivos efetivamente produzidos pelo
    \textit{pipeline}, a conformidade de esquema, faixas de valores plausíveis, ausência de
    nulos críticos, coerência CSV $\leftrightarrow$ GEXF e plausibilidade estatística
    (densidade, modularidade, componente principal).
    
    A cobertura total é de 81\%, com o núcleo analítico (\texttt{core/}) atingindo
    $\geq$ 93\% em todos os submódulos. Os testes de integridade de \textit{datasets}
    ...
    Cabe uma ressalva de transparência sobre esses números. Dos 217 testes, os 64 de
    integridade dependem dos artefatos de dados: só executam após o \textit{pipeline} gerar o
    diretório \texttt{data/}. Em um \textit{checkout} limpo, sem essa geração prévia,
    executam os 153 testes restantes; a cobertura de 81\% pressupõe, portanto, os
    artefatos de dados já produzidos. Trata-se de uma decisão consciente --- validar
    também os dados, e não apenas o código ---, cujo custo é essa dependência de
    ambiente, mitigada pela reprodutibilidade containerizada.
    ...
    \begin{itemize}
        ...
        \item \texttt{docker-compose.yml}: três serviços --- \texttt{pipeline\_chamber} (executa o \textit{pipeline} completo), \texttt{compare} (executa análise comparativa entre anos), \texttt{tests} (executa a suíte de testes);
        \item \texttt{requirements.txt}: lista de bibliotecas Python com versões fixadas.
    \end{itemize}
    
    A execução do \textit{pipeline} completo para os quatro anos (2022--2025) é desencadeada por um único comando: \texttt{docker compose up pipeline\_chamber}. O tempo médio de execução em hardware de classe \textit{desktop} (Intel i5-14600KF, 32\,GB DDR5) é de 15 a 25 minutos.
    
    
    
    \subsection{Orquestração do projeto}
    
    O ambiente pode ser orquestrado totalmente pelo shell \texttt{run.sh}, assim como a execução pode ser feita por esse mesmo shell. Ademais, o comando \texttt{./run.sh help} pode ser de utilidade para saber os comandos específicos do shell.
    
    \subsection{Repositório aberto}

    O código-fonte completo do artefato, juntamente com a documentação técnica e os
    arquivos de empacotamento, está disponível publicamente em repositório aberto
    (\url{https://github.com/fvilhalva/parliament-graph-architecture}), em
    conformidade com os princípios de \textit{Open Science}. A disponibilização do
    código não é um adendo, mas parte constitutiva da contribuição: é o que torna os
    resultados auditáveis, verificáveis e extensíveis por terceiros, fechando o ciclo
    de reprodutibilidade proposto pelo trabalho.

    \subsection{Integração Contínua e Controle de Versão}
    \label{subsec:ci}

    A garantia de qualidade não se restringe à existência da suíte de testes: ela é
    aplicada automaticamente a cada alteração do código, por meio de \textit{Integração
    Contínua} (CI). O repositório define um fluxo de trabalho do \textit{GitHub Actions}
    (\texttt{.github/workflows/ci.yml}) que, a cada \textit{push} ou \textit{pull
    request} destinado aos ramos \texttt{main} e \texttt{develop}, provisiona um
    ambiente limpo com Python 3.11, instala as dependências com versões fixadas
    (\texttt{requirements.txt}), prepara o arquivo de configuração e executa a suíte de
    testes com medição de cobertura, publicando o relatório (\texttt{coverage.xml}) como
    artefato. Cabe uma nota de transparência coerente com a Seção~\ref{subsec:suite}: por
    executar em um \textit{checkout} limpo, sem os artefatos de dados, a CI roda o
    subconjunto de testes independentes de \texttt{data/} (os 153 que não dependem do
    \textit{pipeline}); a suíte completa, com os testes de integridade de \textit{dataset},
    é exercida quando os dados já foram gerados.

    O desenvolvimento seguiu, ainda, um fluxo de ramificação disciplinado ---
    \texttt{main} $\rightarrow$ \texttt{develop} $\rightarrow$ \texttt{feat/*} ---, com a
    integração de cada funcionalidade mediada por \textit{pull requests}. Esse fluxo é
    reforçado por regras de proteção configuradas no repositório (\textit{rulesets}): o
    ramo \texttt{main} não aceita envio direto --- toda integração exige um \textit{pull
    request} cuja verificação de CI (o \textit{status check} da suíte de testes) tenha
    sido bem-sucedida ---, além de bloquear reescrita de histórico (\textit{force-push})
    e exclusão do ramo. Em conjunto, CI e proteção de ramo asseguram, por construção, que
    não seja possível integrar ao ramo principal código que não passe na verificação
    automatizada --- uma evidência concreta do compromisso com a reprodutibilidade e a
    ...
    Esta seção detalha a validação da arquitetura e a reprodutibilidade do artefato de software.
    ...
    O \textit{pipeline} completo (quatro anos) executa em 15 a 25 minutos em hardware de classe \textit{desktop} (Intel i5-14600KF, 32\,GB DDR5), de forma determinística: a fixação da semente aleatória (\texttt{seed}=42) garante que execuções repetidas produzam resultados idênticos. A suíte de 217 testes automatizados (cobertura total de 81\%, núcleo $\geq$ 93\%) valida a corretude de cada módulo. O empacotamento em imagem Docker replica o ambiente de execução de forma exata --- mesma versão de Python, das bibliotecas e do sistema base ---, de modo que qualquer máquina com Docker reproduz integralmente os números reportados neste capítulo, satisfazendo o requisito de reprodutibilidade estrita do artefato.
    ...
    Cada execução persiste os resultados em formatos abertos e interoperáveis: CSV (métricas por deputado), GEXF (grafo, compatível com o Gephi), SQLite (persistência consultável) e JSON (\texttt{AnalysisResult} auditável). O formato GEXF cumpre um papel adicional de \emph{validação cruzada independente}: ao importar o grafo no Gephi e recalcular a modularidade com a implementação própria da ferramenta, obtêm-se valores de $Q$ compatíveis com os produzidos pelo \textit{pipeline}, o que confirma que os resultados não são artefato de uma única implementação. A pluralidade de formatos assegura, ainda, que os dados possam ser auditados e reanalisados por terceiros sem depender do código-fonte original.

    \subsection{Avaliação da Arquitetura: Desacoplamento e Extensibilidade}

    % NORTE: avaliar as QUALIDADES DE PROJETO do artefato (contribuição DSR), não só que "roda".
    Coerentemente com o paradigma DSR --- em que o artefato é a própria contribuição ---, avaliam-se aqui as qualidades estruturais do projeto, e não apenas seu desempenho. Três propriedades da arquitetura, afirmadas no Capítulo~\ref{cap:desenvolvimento}, confirmam-se de forma objetiva.

    Primeiro, o \emph{desacoplamento} é verificável no próprio código: o núcleo analítico (\texttt{core/}) não importa nenhuma das camadas externas (extração, processamento, repositório ou visualização), respeitando estritamente a Regra da Dependência. Esse isolamento não é um detalhe estético --- é o que torna o núcleo \emph{testável de forma independente} de banco de dados, arquivos ou rede, e explica por que a cobertura de testes do \texttt{core/} atinge $\geq 93\%$: a alta testabilidade é consequência direta do design, e não um esforço acidental.

    Segundo, a \emph{extensibilidade} decorre da inversão de dependência (Seção~\ref{subsec:solid}), materializada pelos contratos da camada de persistência (Listagem~\ref{lst:interfaces}). Como o \textit{pipeline} depende das abstrações, e não das implementações concretas, substituir um formato de saída --- por exemplo, trocar o CSV por Parquet, ou o SQLite por outro banco --- exige apenas fornecer uma nova classe que satisfaça o mesmo contrato, sem qualquer alteração no núcleo analítico. O artefato é, portanto, extensível por construção, e não apenas em intenção.

    Terceiro, a \emph{corretude é reimposta automaticamente}: a integração contínua (Seção~\ref{subsec:ci}) executa a suíte de testes a cada alteração, e a proteção do ramo principal impede a integração de código que não passe nessa verificação. A garantia de qualidade deixa de depender de disciplina manual e passa a ser assegurada pelo próprio fluxo de desenvolvimento.

    Em conjunto, essas três evidências demonstram que a arquitetura não é apenas descrita, mas \emph{avaliada}: o artefato realiza os requisitos de modularidade, testabilidade e extensibilidade que motivaram seu projeto --- o que constitui, no paradigma DSR, a demonstração de sua eficácia.

    \section{ANÁLISE DE CENTRALIDADE E INFLUÊNCIA PARLAMENTAR}

    Esta seção caracteriza a rede de coautoria produzida pelo artefato em dois planos. Primeiro, descreve-se a estrutura global do grafo em cada ano e o efeito empírico dos filtros de modelagem; em seguida, identificam-se os deputados que ocupam as posições estruturalmente mais centrais e discute-se o que essas posições revelam --- e o que \emph{não} revelam --- sobre a articulação legislativa.
    ...
    A remoção de 70 proposições com mais de 30 coautores --- a maior com 226 signatários (uma PEC) --- reduz a densidade de 85,35\% para 9,90\%, transformando um grafo quase-completo em uma rede analisável. Vale reforçar que a heurística de ponderação $1/(n_p-1)$ e o filtro de massa atacam distorções \emph{distintas e complementares}: a normalização corrige a distorção de \emph{peso} (cada par de uma PEC de massa contribui apenas $1/225 \approx 0{,}004$ à força do vínculo), enquanto o filtro corrige a distorção \emph{topológica} --- a mera \emph{existência} das dezenas de milhares de arestas de um clique de signatários, que as métricas topológicas (intermediação, proximidade) e a própria densidade não conseguem ignorar, por mais baixo que seja o peso.
    
    A robustez do limiar de 30 e a necessidade do filtro foram avaliadas por uma análise de sensibilidade (Tabela~\ref{tab:sensibilidade}), variando \texttt{max\_authors} em 2025.
    
    \begin{table}[H]
    \centering
    \caption{Sensibilidade da rede ao limiar \texttt{max\_authors} (2025).}
    \label{tab:sensibilidade}
    \begin{tabular}{lrrr}
    \toprule
    \texttt{max\_authors} & \textbf{Arestas} & \textbf{Densidade} & \textbf{$Q$ Louvain} \\
    \midrule
    20                 & 2.415   & 5,8\%  & 0,699 \\
    30 (adotado)       & 5.407   & 9,9\%  & 0,634 \\
    40                 & 7.521   & 12,0\% & 0,601 \\
    Sem filtro         & 124.671 & 85,4\% & 0,215 \\
    \bottomrule
    \end{tabular}
    \fonte{Elaborado pelo autor.}
    \end{table}
    
    Dois resultados sobressaem. Primeiro, a modularidade é \emph{estável} na faixa de limiares plausíveis (20 a 40), mantendo-se sempre bem acima do patamar de estrutura significativa ($Q > 0{,}3$); a escolha de 30 é, portanto, um meio-termo conservador que \emph{não determina} o resultado. Segundo, e mais importante, sem o filtro --- e mesmo com a normalização ativa --- a modularidade \emph{colapsa de $0{,}634$ para $0{,}215$}: a prova empírica de que o filtro \emph{não é redundante} com a ponderação, pois trata uma distorção (a topológica) que a ponderação não alcança. Os valores das Tabelas~\ref{tab:filtros} e~\ref{tab:sensibilidade} são gerados de forma reprodutível pelo utilitário \texttt{scripts/sensitivity.py} (comando \texttt{./run.sh sensitivity}), que reconstrói a rede sob cada configuração de \texttt{max\_authors}.
    ...
    A Tabela~\ref{tab:top_deputados} apresenta os deputados de maior centralidade em 2025, sob duas óticas: intermediação (pontes entre grupos) e grau ponderado (intensidade de colaboração).\footnote{Por se tratar de período eleitoral, os nomes de parlamentares e as siglas partidárias são apresentados de forma anonimizada (rótulos genéricos, locais a cada tabela) ao longo deste capítulo. A análise é estritamente estrutural e independe da identidade das legendas; a correspondência real é recuperável a partir dos dados abertos processados pelo artefato.}

    \begin{table}[H]
    \centering
    \caption{Deputados de maior centralidade em 2025 (identidades anonimizadas).}
    \label{tab:top_deputados}
    \begin{tabular}{ll}
    \toprule
    \textbf{Top intermediação (pontes)} & \textbf{Top grau ponderado (coesão)} \\
    \midrule
    Deputado 1 (Partido A) & Deputado 6 (Partido E)  \\
    Deputado 2 (Partido B) & Deputado 7 (Partido F)  \\
    Deputado 3 (Partido B) & Deputado 8 (Partido E)  \\
    Deputado 4 (Partido C) & Deputado 9 (Partido F)  \\
    Deputado 5 (Partido D) & Deputado 10 (Partido B) \\
    \bottomrule
    \end{tabular}
    \fonte{Elaborado pelo autor.}
    \end{table}
    ...
    O primeiro padrão diz respeito às \emph{pontes}. Os deputados de maior intermediação distribuem-se por \emph{quatro partidos distintos}, e o principal \textit{broker} não pertence ao maior bloco. O papel de ponte entre grupos é, portanto, uma posição \emph{estrutural e individual}, não determinada pelo tamanho da bancada: um parlamentar de partido médio pode ocupar posição de mediação mais central que membros de blocos numerosos.
    
    O segundo padrão é o mais revelador. O grau ponderado é dominado por deputados de \emph{dois partidos pequenos e coesos}, que coassinam intensamente entre si. Isso evidencia que a centralidade de coautoria mede \emph{intensidade de articulação}, e \emph{não poder institucional}. Retomando a definição operacional adotada (Seção~\ref{subsec:metricas}), influência estrutural é a posição na rede, não o controle de relatorias, da presidência de comissões ou da pauta --- mecanismos que se exercem por fora da coautoria. É por isso que lideranças formais e presidentes de comissão \emph{não} aparecem no topo das métricas: sua influência opera por canais que a rede de coautoria, por construção, não captura. A rede revela quem \emph{articula muito}, não necessariamente quem \emph{decide muito}. Responde-se, assim, à terceira pergunta norteadora: a centralidade de coautoria \emph{não} prediz influência política no sentido institucional --- ela mede articulação e colaboração, dimensão estruturalmente distinta do poder decisório.
    
    % NORTE: responde a pergunta norteadora (i) --- concentração da articulação em poucos.
    A concentração da articulação em poucos parlamentares --- primeira pergunta norteadora --- é quantificada pelo coeficiente de \citeonline{gini1921} da distribuição de cada centralidade (0 indica igualdade total; 1, concentração máxima) e pela participação dos dez deputados mais centrais. A Tabela~\ref{tab:concentracao} apresenta os valores de 2025.
    ...
    \begin{figure}[H]
        \centering
        \caption{Curvas de Lorenz da concentração das centralidades em 2025: quanto mais a curva se afasta da diagonal de igualdade, maior a concentração da métrica em poucos parlamentares.}
        \label{fig:concentracao_gini}
        \includegraphics[width=0.7\textwidth]{imagens/resultados/2025/concentration_gini.png}
        \fonte{Elaborado pelo autor.}
    \end{figure}
    
    Os valores respondem afirmativamente à primeira pergunta norteadora: há forte concentração da articulação em poucos parlamentares, com intensidade que \emph{varia} conforme a dimensão medida. A concentração é \emph{extrema no autovetor} (Gini $0{,}951$; os dez deputados mais centrais concentram 82\% da métrica) --- reflexo direto das bancadas coesas que coassinam quase exclusivamente entre si, formando câmaras de eco; \emph{alta na intermediação} ($0{,}740$), pois o papel de ponte é raro; e \emph{moderada no grau ponderado} ($0{,}566$), a mais distribuída das quatro. Cabe a ressalva já discutida: trata-se de concentração da \emph{articulação} (coautoria), não de poder institucional. O padrão é estável ao longo do período (o Gini do autovetor permanece entre $0{,}87$ e $0{,}98$ nos quatro anos). Por fim, a própria divergência entre os coeficientes ($0{,}57$, $0{,}74$ e $0{,}95$) confirma que as quatro métricas de centralidade capturam dimensões distintas de influência --- não são redundantes ---, o que justifica reportá-las em conjunto. A~\ref{fig:concentracao_gini} representa graficamente essa concentração por meio das curvas de Lorenz das quatro centralidades: quanto mais distante da diagonal de igualdade, mais concentrada a métrica.
    
    \section{DETECÇÃO DE COMUNIDADES E TESTE DA HIPÓTESE CENTRAL}

    Esta seção examina a organização da rede em comunidades e submete a hipótese central (H1) ao teste estatístico. Avalia-se primeiro a robustez das partições, confrontando os dois algoritmos de detecção; em seguida, testa-se a significância da estrutura comunitária contra o modelo nulo ponderado e verifica-se sua correspondência com a filiação partidária.
    ...
    A concordância é \emph{moderada} (ARI entre $0{,}281$ e $0{,}423$). Esse valor não deve ser lido como fragilidade, mas como o que de fato é: os dois algoritmos \emph{convergem no essencial} --- ambos detectam forte estrutura modular na mesma rede ---, divergindo apenas na \emph{granularidade} das fronteiras, isto é, em como partem os deputados situados entre blocos e em quantas comunidades reconhecem (o Label Propagation, reconhecidamente mais instável, varia de 10 a 19 comunidades). O ARI funciona, assim, como uma \emph{verificação de consistência do sinal}: se os dois métodos concordassem apenas ao acaso (ARI $\approx 0$), a estrutura detectada seria suspeita; a concordância moderada e positiva indica um sinal estrutural real, ainda que suas bordas dependam do algoritmo. Não se trata, portanto, de exigir partições idênticas, mas de confirmar que a fragmentação em comunidades é robusta à escolha do método.
    ...
    Em todos os anos, a modularidade observada supera a média do modelo nulo, e \emph{nenhum dos 200 grafos nulos atinge $Q_{\text{obs}}$} ($p<0{,}005$). \emph{A hipótese H1 é, portanto, corroborada}: a estrutura de comunidades da rede é não-aleatória. Ressalte-se que o modelo nulo é \emph{ponderado} --- os pesos originais são reatribuídos às arestas reconfiguradas ---, de modo que a comparação é homogênea com a modularidade observada, também ponderada. Corroborada H1, cria-se a \emph{condição de validade} que autoriza a etapa seguinte: interpretar as comunidades detectadas como estruturas reais, e não como ruído. É sobre essa base que se responde à segunda pergunta norteadora.
    
    % NORTE: aqui vai o "ouro" --- mostrar que as comunidades correspondem a partidos.
    A correspondência entre as comunidades detectadas e a filiação partidária foi avaliada de forma estritamente estrutural, sem recurso a qualquer classificação ideológica. A Tabela~\ref{tab:comunidade_partido} apresenta a composição das principais comunidades de 2025.
    
    \begin{table}[H]
    \centering
    \caption{Composição partidária das principais comunidades (Louvain, 2025; partidos anonimizados).}
    \label{tab:comunidade_partido}
    \begin{tabular}{crl}
    \toprule
    \textbf{Comunidade} & \textbf{Deputados} & \textbf{Partido dominante (\%) + aliados} \\
    \midrule
    1 & 118 & Partido A (54\%) + B, C, D \\
    3 & 63  & Partido E (59\%) + F \\
    9 & 47  & Partido G (30\%) + H, E \\
    4 & 33  & Partido I (39\%) + E, J \\
    6 & 24  & Partido D (25\%) + C, K \\
    7 & 6   & Partido L (67\%) \\
    \bottomrule
    \end{tabular}
    \fonte{Elaborado pelo autor.}
    ...
    O algoritmo \emph{recupera a organização partidária} sem jamais receber rótulo de partido: cada comunidade é dominada por uma legenda real. Contudo, a correspondência é \emph{parcial, e não idêntica} --- medida pelo \textit{Adjusted Rand Index} entre a partição de comunidades e a partição por filiação, obtém-se ARI $\approx 0{,}30$ e pureza partidária $\approx 48\%$ (proporção média de deputados no partido dominante de sua comunidade). Isso significa que as comunidades são \emph{mais amplas que partidos isolados}: enquanto algumas correspondem a um partido coeso (a comunidade~3, essencialmente de uma única legenda), outras agregam um partido dominante e legendas estruturalmente aliadas por coautoria (a comunidade~1 reúne um partido dominante e três legendas aliadas). Respondendo à pergunta norteadora, as comunidades correspondem, portanto, \emph{mais a agrupamentos do tipo coalizão do que a linhas partidárias puras}. Essa leitura permanece no plano estrutural: descreve-se \emph{quais partidos se agrupam por coautoria} --- um fato observável ---, sem atribuir-lhes rótulos de posição ideológica. O \textit{Adjusted Rand Index} e a pureza aqui reportados são calculados de forma reprodutível por \texttt{scripts/compare\_years.py}, que cruza a partição de comunidades (Louvain) com a filiação partidária a partir das métricas por deputado exportadas pelo \textit{pipeline}.
    
    A~\ref{fig:gephi_comunidades_partido} oferece a validação visual dessa correspondência. As duas imagens usam \emph{o mesmo posicionamento} de nós, gerado pelo algoritmo ForceAtlas~2 a partir exclusivamente das arestas (sem qualquer informação de partido ou comunidade). À esquerda, a coloração por comunidade (Louvain); à direita, pela filiação partidária. A concordância espacial entre as duas colorações confirma, de forma independente do algoritmo de detecção, que os agrupamentos são reais: onde a comunidade é um partido coeso, as cores coincidem; onde é uma coalizão, um único bloco espacial aparece multicolorido no painel de partidos --- exatamente o comportamento previsto pelo ARI moderado.
    
    \begin{figure}[H]
        \centering
        \caption{Legenda de cores por comunidade (Louvain) da~\ref{fig:gephi_comunidades_partido}. A legenda de cores por partido foi omitida em razão do período eleitoral.}
        \label{fig:gephi_legenda_comunidades_partido}
        \includegraphics[width=0.5\textwidth]{imagens/resultados/gephi/legenda-comunidades.png}
        \fonte{Elaborado pelo autor com o Gephi.}
    \end{figure}
    
    \begin{figure}[H]
        \centering
        \caption{Rede de coautoria de 2025 (mesmo \textit{layout} ForceAtlas~2), colorida por comunidade detectada (a) e por filiação partidária (b). Arestas com peso baixo omitidas para legibilidade.}
        \label{fig:gephi_comunidades_partido}
        \begin{minipage}{0.7\textwidth}
            \centering
            \includegraphics[width=\linewidth]{imagens/resultados/gephi/rede-comunidades-2025-02.png}
            \\{\small (a) Cor por comunidade (Louvain)}
        \end{minipage}
        \hfill
        \begin{minipage}{0.7\textwidth}
            \centering
            \includegraphics[width=\linewidth]{imagens/resultados/gephi/rede-partidos-2025-03.png}
            \\{\small (b) Cor por partido}
        \end{minipage}
        \fonte{Elaborado pelo autor com o Gephi.}
    \end{figure}
    ...
    Tendo caracterizado a rede em detalhe para 2025, esta seção avalia se o padrão observado se sustenta ao longo dos quatro anos analisados. Discute-se primeiro a estabilidade das métricas estruturais e, na sequência, a leitura longitudinal do período, com as ressalvas metodológicas que o recorte impõe.

    \subsection{Estabilidade do Padrão Estrutural}
    
    % NORTE: mostrar que Q e o veredito de H1 se mantêm nos 4 anos (persistência).
    A caracterização detalhada foi apresentada para 2025, ano mais recente e representativo do regime estável da 57\textsuperscript{a} Legislatura. Esta seção demonstra que o padrão descrito \emph{se mantém em todo o período}. A modularidade observada permanece em uma faixa estreita, $Q_{\text{obs}} \in [0{,}596;\ 0{,}640]$, e H1 é corroborada nos quatro anos ($p<0{,}005$; Tabela~\ref{tab:null_model}). A fragmentação em comunidades é, portanto, uma propriedade \emph{persistente} do legislativo no período, e não um artefato de um ano específico. A~\ref{fig:tendencias} sintetiza a evolução das principais métricas estruturais.
    
    \begin{figure}[H]
        \centering
        \caption{Evolução das métricas estruturais (2022--2025): modularidade, ARI, densidade e concentração (Gini do autovetor).}
        \label{fig:tendencias}
        \includegraphics[width=0.9\textwidth]{imagens/resultados/analise_temporal/compare_temporal_trends.png}
        \fonte{Elaborado pelo autor.}
    \end{figure}
    
    Observa-se estabilidade nas quatro dimensões: a modularidade oscila pouco em torno de $0{,}62$; a concentração no autovetor mantém-se muito alta (Gini $> 0{,}87$); e a densidade sobe ligeiramente em 2025 ($9{,}9\%$), acompanhando o maior número de coautorias registradas naquele ano.
    
    \subsection{Discussão Longitudinal}
    
    % NORTE: a ressalva metodológica do recorte + leitura política prudente.
    Uma ressalva metodológica é necessária. O ano de 2022 pertence à \emph{56\textsuperscript{a} Legislatura} --- em seu encerramento e em ano eleitoral ---, enquanto 2023 a 2025 compõem a \emph{57\textsuperscript{a}}. Assim, 2022 deve ser lido como ponto de referência para avaliar a \emph{persistência} do padrão através de uma transição de legislatura, e não como termo de uma comparação ponto-a-ponto homogênea; a leve variação em número de nós e arestas entre os anos reflete, em parte, essa descontinuidade institucional. Ainda assim, o achado central sobrevive à ressalva: a estrutura modular significativa se mantém \emph{apesar} da mudança de legislatura, o que sugere que a organização da colaboração legislativa em blocos é uma característica estável do funcionamento da Câmara, e não um efeito conjuntural. Essa leitura é deliberadamente prudente: a coautoria é um \textit{proxy} de articulação, e o que se afirma limita-se ao que essa evidência estrutural autoriza --- a persistência de uma organização em comunidades ---, sem extrapolar para conclusões sobre coesão de voto ou disputa ideológica, que exigiriam outras fontes de dados.
    ...
    Este capítulo encerra o trabalho. Retoma-se o problema de pesquisa e sintetizam-se os achados; sistematizam-se, então, as contribuições técnica e analítica do artefato; e, por fim, reconhecem-se com honestidade as limitações do estudo e apontam-se caminhos para trabalhos futuros.

    \section{SÍNTESE DOS ACHADOS}
    
    % NORTE: 1 parágrafo respondendo ao problema de pesquisa.
    O objetivo geral deste trabalho --- projetar, implementar e validar uma arquitetura de software reprodutível para análise de redes parlamentares --- foi cumprido. Construiu-se um artefato modular e auditável que transforma dados legislativos brutos da Câmara dos Deputados em evidências analíticas mensuráveis, e sua eficácia foi demonstrada sobre o período 2022--2025. A hipótese central (H1) foi \emph{corroborada nos quatro anos} ($p<0{,}005$): a estrutura de comunidades da rede de coautoria é estatisticamente não-aleatória, propriedade que se mostrou persistente. Sobre essa base, as centralidades e as comunidades caracterizaram as \emph{estruturas de influência} em seus dois níveis --- os blocos (comunidades, correspondentes a coalizões partidárias) e os atores centrais (pontes e polos de articulação). O achado analítico mais relevante é a \emph{distinção empírica entre articulação e poder institucional}: a centralidade de coautoria mede a intensidade da colaboração, não o controle das decisões, e essa fronteira foi tornada explícita pelos dados.
    
    \section{CONTRIBUIÇÕES}

    As contribuições do trabalho organizam-se em duas frentes complementares, coerentes com o paradigma DSR: a contribuição \emph{técnica} --- o artefato em si --- e a contribuição \emph{analítica} --- o que sua aplicação ao período 2022--2025 revelou.

    \subsection{Contribuição técnica}
    
    % NORTE: o artefato em si.
    A contribuição central e primária do trabalho, coerente com o paradigma DSR, é o \emph{próprio artefato}: uma arquitetura de software modular, reprodutível e auditável para análise de redes parlamentares. Seguindo os princípios de Arquitetura Limpa e SOLID, o \textit{pipeline} isola rigorosamente o núcleo analítico das preocupações de entrada e saída, é coberto por 217 testes automatizados --- incluindo testes de integridade sobre os próprios \textit{datasets} gerados --- e é empacotado em Docker, garantindo execução idêntica em qualquer ambiente. O desacoplamento é reforçado por interfaces (contratos) na camada de persistência, que aplicam a inversão de dependência e tornam o artefato extensível a novos formatos sem alterar o núcleo; e sua corretude é reimposta automaticamente por integração contínua a cada alteração do código. Trata-se de uma \emph{ferramenta reutilizável} e independente da fonte, disponibilizada em código aberto, apta a ser estendida para outras legislaturas, outros recortes ou outras casas legislativas.
    
    \subsection{Contribuição analítica}
    
    % NORTE: o que a aplicação revelou.
    Como contribuição secundária --- a demonstração de que o artefato funciona ---, o trabalho oferece uma caracterização estrutural do legislativo brasileiro no período 2022--2025: (i) a existência de estrutura modular não-aleatória e persistente; (ii) comunidades que recuperam a organização partidária, ainda que no nível de coalizão e não de partido isolado; (iii) forte concentração da articulação em poucos parlamentares, especialmente nas bancadas coesas; e (iv) a já mencionada distinção entre a articulação capturada pela coautoria e o poder institucional formal. Essas descrições ilustram o tipo de evidência que a arquitetura é capaz de produzir de forma reprodutível.
    
    \section{LIMITAÇÕES}
    
    O reconhecimento explícito das limitações é parte da honestidade científica que o trabalho busca manter. As principais são:
    
    \begin{itemize}
        \item \textbf{Coautoria como \textit{proxy}}: a rede mede articulação, não coesão de voto nem posição ideológica formal; dois adversários podem coassinar um projeto pontual, e aliados podem divergir em votações.
        \item \textbf{Centralidade $\neq$ poder institucional}: a rede não captura relatorias, presidência de comissões ou controle de pauta --- canais de poder que operam por fora da coautoria.
        \item \textbf{Correspondência parcial com partidos}: as comunidades recuperam a organização partidária apenas no nível de coalizão (ARI $\approx 0{,}30$), não de partido isolado.
        \item \textbf{ARI moderado} (0,28--0,42) entre Louvain e Label Propagation: os algoritmos convergem no essencial, mas divergem na granularidade das fronteiras.
        \item \textbf{Recorte temporal}: 2022 pertence à 56\textsuperscript{a} Legislatura (ano eleitoral atípico), o que limita a comparação ponto-a-ponto com os demais anos.
        \item \textbf{Escopo dos filtros}: embora a modularidade tenha se mostrado robusta ao limiar \texttt{max\_authors} (Seção~\ref{subsec:heuristica}), o filtro exclui integralmente as PECs da rede analisada; a exclusão de um tipo inteiro de proposição é uma decisão de escopo, adequada à definição de articulação adotada, mas que restringe o alcance da análise.
        \item \textbf{Acoplamento à fonte}: o artefato depende do esquema de arquivos CSV do Portal de Dados Abertos da Câmara; mudanças nesse esquema exigiriam adaptação da camada de extração.
    \end{itemize}
    ...
    \begin{itemize}
        \item Análise de sensibilidade do filtro de tipos de proposição --- em especial o impacto da inclusão das emendas (EMC) na estrutura ---, complementando a análise do limiar \texttt{max\_authors} já realizada.
        \item Comparação com o algoritmo de Leiden \citeonline{traag2019}, que garante comunidades bem conectadas.
        ...
        \item Desenvolvimento de uma visualização interativa da rede --- por exemplo, um painel web navegável --- que permita explorar comunidades, centralidades e a evolução temporal de forma dinâmica, complementando as figuras estáticas geradas pelo artefato e a inspeção no Gephi.
        \item Aplicação de Redes Neurais de Grafos (GNNs) para predição de comportamento legislativo.
        \item Análise textual do conteúdo legislativo: os metadados brutos já extraídos incluem a ementa de cada proposição, permitindo, em trabalho futuro, técnicas de Processamento de Linguagem Natural (classificação temática, extração de tópicos) para investigar se comunidades e deputados centrais se concentram em pautas específicas.
    \end{itemize}
    
    \section{CONSIDERAÇÕES FINAIS}
    
    % NORTE: fechamento curto, retomando o valor do artefato (transparência/auditoria).
    Ao unir engenharia de software contemporânea e método científico, este trabalho demonstrou que é possível transformar a abundância de dados legislativos abertos em evidências estruturais mensuráveis, reprodutíveis e auditáveis. Mais do que os achados específicos sobre o período 2022--2025, o resultado duradouro é a \emph{ferramenta} construída: um instrumento de código aberto que aproxima a análise de redes da realidade da Câmara dos Deputados e que fica à disposição para auditoria, extensão e reuso. Em um contexto que demanda transparência e escrutínio do processo legislativo, disponibilizar não apenas conclusões, mas o próprio meio reprodutível de chegar a elas, é a contribuição que se espera perdurar.
    ...
    %-------------------------------------------------------------------------------------%
        \begin{apendicesenv}
            ...
            \url{https://github.com/fvilhalva/parliament-graph-architecture}. O
            ...
            \begin{itemize}
                ...
                \item \texttt{src/tests/}: suíte de testes (217 testes);
