...
\label{cap:revisao}
    ...
    Explicita-se, para evitar ambiguidade, a natureza do objeto matemático analisado: a rede de coautoria $G$ é um \textbf{grafo simples} (sem laços ou arestas múltiplas --- coautorias repetidas entre o mesmo par acumulam-se no peso, não em arestas paralelas), \textbf{não-direcionado} (a coautoria é uma relação simétrica) e \textbf{ponderado} (cada aresta carrega o peso $w(i,j)$ da Equação~\ref{eq:peso_coautoria}). Cada ano é modelado como um \textit{snapshot} \textbf{estático} independente; a dimensão temporal do estudo (Seção~\ref{sec:temporal}) não decorre de um grafo dinâmico único, mas da \emph{comparação} entre os quatro grafos anuais construídos de forma idêntica.
    ...
    Cada execução persiste os resultados em formatos abertos e interoperáveis: CSV (métricas por deputado), GEXF (grafo, compatível com o Gephi), SQLite (persistência consultável) e JSON (\texttt{AnalysisResult} auditável). O formato GEXF cumpre um papel adicional de \textbf{validação cruzada independente}: ao importar o grafo no Gephi e recalcular a modularidade com a implementação própria da ferramenta, obtêm-se valores de $Q$ compatíveis com os produzidos pelo \textit{pipeline}, o que confirma que os resultados não são artefato de uma única implementação. A pluralidade de formatos assegura, ainda, que os dados possam ser auditados e reanalisados por terceiros sem depender do código-fonte original.
    ...
    Dois resultados sobressaem. Primeiro, a modularidade é \textbf{estável} na faixa de limiares plausíveis (20 a 40), mantendo-se sempre bem acima do patamar de estrutura significativa ($Q > 0{,}3$); a escolha de 30 é, portanto, um meio-termo conservador que \textbf{não determina} o resultado. Segundo, e mais importante, sem o filtro --- e mesmo com a normalização ativa --- a modularidade \textbf{colapsa de $0{,}634$ para $0{,}215$}: a prova empírica de que o filtro \emph{não é redundante} com a ponderação, pois trata uma distorção (a topológica) que a ponderação não alcança. Os valores das Tabelas~\ref{tab:filtros} e~\ref{tab:sensibilidade} são gerados de forma reprodutível pelo utilitário \texttt{scripts/sensitivity.py} (comando \texttt{./run.sh sensitivity}), que reconstrói a rede sob cada configuração de \texttt{max\_authors}.
    ...
    O primeiro padrão diz respeito às \textbf{pontes}. Os deputados de maior intermediação distribuem-se por partidos distintos --- REPUBLICANOS, PL, MDB e PP ---, e o principal \textit{broker} não pertence ao maior bloco. O papel de ponte entre grupos é, portanto, uma posição \textbf{estrutural e individual}, não determinada pelo tamanho da bancada: um parlamentar de partido médio pode ocupar posição de mediação mais central que membros de blocos numerosos.
    
    O segundo padrão é o mais revelador. O grau ponderado é dominado por deputados de partidos pequenos e coesos --- PSOL e NOVO ---, que coassinam intensamente entre si. Isso evidencia que a centralidade de coautoria mede \textbf{intensidade de articulação}, e \textbf{não poder institucional}. Retomando a definição operacional adotada (Seção~\ref{subsec:metricas}), influência estrutural é a posição na rede, não o controle de relatorias, da presidência de comissões ou da pauta --- mecanismos que se exercem por fora da coautoria. É por isso que lideranças formais e presidentes de comissão \emph{não} aparecem no topo das métricas: sua influência opera por canais que a rede de coautoria, por construção, não captura. A rede revela quem \emph{articula muito}, não necessariamente quem \emph{decide muito}.
    ...
    A concordância é \textbf{moderada} (ARI entre $0{,}281$ e $0{,}423$). Esse valor não deve ser lido como fragilidade, mas como o que de fato é: os dois algoritmos \textbf{convergem no essencial} --- ambos detectam forte estrutura modular na mesma rede ---, divergindo apenas na \emph{granularidade} das fronteiras, isto é, em como partem os deputados situados entre blocos e em quantas comunidades reconhecem (o Label Propagation, reconhecidamente mais instável, varia de 10 a 19 comunidades). O ARI funciona, assim, como uma \textbf{verificação de consistência do sinal}: se os dois métodos concordassem apenas ao acaso (ARI $\approx 0$), a estrutura detectada seria suspeita; a concordância moderada e positiva indica um sinal estrutural real, ainda que suas bordas dependam do algoritmo. Não se trata, portanto, de exigir partições idênticas, mas de confirmar que a fragmentação em comunidades é robusta à escolha do método.
    ...
    O objetivo geral deste trabalho --- projetar, implementar e validar uma arquitetura de software reprodutível para análise de redes parlamentares --- foi cumprido. Construiu-se um artefato modular e auditável que transforma dados legislativos brutos da Câmara dos Deputados em evidências analíticas mensuráveis, e sua eficácia foi demonstrada sobre o período 2022--2025. A hipótese central (H1) foi \textbf{corroborada nos quatro anos} ($p<0{,}005$): a estrutura de comunidades da rede de coautoria é estatisticamente não-aleatória, propriedade que se mostrou persistente. Sobre essa base, as centralidades e as comunidades caracterizaram as \textbf{estruturas de influência} em seus dois níveis --- os blocos (comunidades, correspondentes a coalizões partidárias) e os atores centrais (pontes e polos de articulação). O achado analítico mais relevante é a \textbf{distinção empírica entre articulação e poder institucional}: a centralidade de coautoria mede a intensidade da colaboração, não o controle das decisões, e essa fronteira foi tornada explícita pelos dados.
    ...
    A contribuição central e primária do trabalho, coerente com o paradigma DSR, é o \textbf{próprio artefato}: uma arquitetura de software modular, reprodutível e auditável para análise de redes parlamentares. Seguindo os princípios de Arquitetura Limpa e SOLID, o \textit{pipeline} isola rigorosamente o núcleo analítico das preocupações de entrada e saída, é coberto por 217 testes automatizados --- incluindo testes de integridade sobre os próprios \textit{datasets} gerados --- e é empacotado em Docker, garantindo execução idêntica em qualquer ambiente. Trata-se de uma \textbf{ferramenta reutilizável} e independente da fonte, disponibilizada em código aberto, apta a ser estendida para outras legislaturas, outros recortes ou outras casas legislativas.
